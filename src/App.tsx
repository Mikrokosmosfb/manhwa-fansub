...
const MainContent: React.FC = () => {
  const { view, setView, seriesList } = useApp();
  ...
  const novelSeries = seriesList.filter(s => s.type === 'Web Novel');
  const comicSeries = seriesList.filter(s => s.type !== 'Web Novel');

  // Popular tabs mock data sorting
  const popularSeries = [...seriesList].sort((a, b) => b.rating - a.rating);
    ...
    <div className="space-y-8">
      ...
      <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <section className="bg-gray-900/90 border border-purple-500/20 rounded-3xl p-5 shadow-xl relative">
            <div className="flex items-center justify-between pb-3 mb-4 border-b border-gray-800">
              ...
              <div className="flex items-center gap-2">
                <div className="flex items-center gap-1 bg-gray-950 p-1 rounded-xl border border-purple-500/20">
                  <button
                ...
                <button
                  ...
                  className="text-xs font-bold text-purple-300 hover:text-white bg-purple-950/80 hover:bg-purple-900 border border-purple-800 px-3 py-1 rounded-xl transition flex items-center gap-1"
                >
                  Tümünü Gör
            ...
            <div
              ref={novelScrollRef}
              className="flex gap-4 overflow-x-auto no-scrollbar scroll-smooth snap-x pb-2 pt-1"
            >
              {novelSeries.length === 0 ? (
                ...
                novelSeries.map(s => (
                  <div key={s.id} className="flex-shrink-0 w-36 sm:w-44 snap-start">
          ...
          <section className="bg-gray-900/90 border border-purple-500/20 rounded-3xl p-5 shadow-xl">
            <div className="flex items-center justify-between pb-3 mb-4 border-b border-gray-800">
              <h2 className="text-base sm:text-lg font-extrabold text-white flex items-center gap-2">
        ...
        <div className="space-y-6">
          ...
          <section className="bg-gray-900/90 border border-purple-500/20 rounded-3xl p-5 shadow-xl space-y-4">
...
const AppLayout: React.FC = () => {
  const { theme, isAuthModalOpen, closeAuthModal, authModalInitialTab } = useApp();
  return (
    <div className={`min-h-screen flex flex-col font-sans selection:bg-purple-600 selection:text-white overflow-x-hidden w-full max-w-full pb-14 md:pb-0 transition-colors duration-200 ${
