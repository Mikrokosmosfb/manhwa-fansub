...
export const AdminNotificationsManager: React.FC = () => {
  ...
  const applyTemplate = (templateType: 'server' | 'event' | 'new_series' | 'discord') => {
    if (templateType === 'server') {
      ...
      if (seriesList.length > 0) {
        ...
        setTitle(`📖 Yeni Seri: ${s.title} Yayında!`);
        setMessage(`${s.title} güncel ve yüksek çözünürlüklü Türkçe çevirisiyle şimdi Mikrokosmos Fansub'da!`);
        setType('chapter');
          ...
          <form onSubmit={handleBroadcastSubmit} className="lg:col-span-7 bg-gray-950/70 border border-purple-500/20 rounded-3xl p-5 sm:p-7 space-y-5 shadow-xl">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                {[
                  { id: 'announcement', label: '📢 Duyuru', desc: 'Genel haber & site duyurusu' },
                  { id: 'system', label: '⚙️ Sistem', desc: 'Güncelleme & bakım' },
                  { id: 'reward', label: '🎁 Ödül / Puan', desc: 'Hediye & etkinlik' },
                  { id: 'chapter', label: '📖 Bölüm Yayını', desc: 'Seri / bölüm haberi' }
                ].map(t => (
                  <button
                    key={t.id}
                    type="button"
                    ...
                    className={`p-3 rounded-2xl border text-left transition ${
                      type === t.id
                        ? 'bg-purple-950/80 border-purple-400 text-white shadow-lg ring-1 ring-purple-400/50'
            ...
            <div>
              <label className="block text-xs font-bold text-gray-300 mb-1.5">
                Özel Görsel / Kapak Resmi URL (Opsiyonel):
              </label>
              <div className="flex gap-2">
                <input
                  ...
                  placeholder="https://images.unsplash.com/... veya görsel bağlantısı"
                  className="flex-1 bg-gray-900 border border-gray-800 focus:border-purple-500 rounded-2xl px-4 py-2.5 text-xs text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-purple-500/20 transition"
                />
                {customCoverImage && (
                  <button
                    type="button"
                    onClick={() => setCustomCoverImage('')}
                    className="px-3 py-2 bg-gray-800 hover:bg-gray-700 text-gray-400 hover:text-white rounded-xl text-xs transition"
                  >
                    Temizle
                  </button>
                )}
