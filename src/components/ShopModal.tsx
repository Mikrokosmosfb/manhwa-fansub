...
export const ShopContent: React.FC = () => {
  ...
  const [activeCategory, setActiveCategory] = useState<'buy_coins' | 'theme_photo' | 'theme_aura' | 'badge' | 'emoji_pack' | 'all'>('buy_coins');
...
export const ShopView: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6 space-y-6 animate-fadeIn">
        ...
        <div className="flex items-center gap-2 bg-gray-950/80 border border-amber-500/40 px-4 py-2 rounded-2xl">
          ...
          <div className="text-right">
            ...
            <span className="text-sm font-black text-amber-300 font-mono">
              {user?.email?.toLowerCase() === 'aseleliyeva77@gmail.com' ? 'Sınırsız CP' : `${(user?.coins ?? 250).toLocaleString('tr-TR')} CP`}
            </span>
          </div>
        </div>
      </div>
      ...
      {/* SHOP CONTENT */}
      <div className="bg-gray-950/90 border border-purple-500/20 rounded-3xl p-4 sm:p-6 shadow-xl">
        <ShopContent />
      </div>
    </div>
  );
