...
export const AdvancedSearchView: React.FC = () => {
  ...
  // Extract all unique genres and tags from GENRE_LIST and all series (including customBadges)
  const allGenres = useMemo(() => {
    const set = new Set<string>();

    // 1. Add all predefined genres
    GENRE_LIST.forEach(g => {
      if (g && g.trim()) {
        const trimmed = g.trim();
        if (!showNsfw && (trimmed === '18+' || trimmed === '21+')) return;
        set.add(trimmed);
      }
    });

    // 2. Add series genres and custom badges across all series
  ...
  const filteredSeries = useMemo(() => {
    return seriesList.filter(s => {
      ...
      // Included genres & tags check (MUST contain all included genres/tags)
      if (includedGenres.length > 0) {
        const hasAllIncluded = includedGenres.every(g => {
          const gLower = g.toLowerCase().trim();
          const inGenres = s.genres?.some(sg => sg.toLowerCase().trim() === gLower);
          const inBadges = s.customBadges?.some(sb => sb.toLowerCase().trim() === gLower);
          return inGenres || inBadges;
        });
        if (!hasAllIncluded) return false;
      }

      // Excluded genres & tags check (MUST NOT contain any excluded genre/tag)
          ...
          <div className="pt-3 border-t border-gray-800 space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              <label className="text-xs font-extrabold text-purple-300 uppercase tracking-wider flex items-center gap-1.5">
                <Filter size={14} /> Tür & Etiket Filtreleme Matrixi ({allGenres.length}) (Tıklayarak Seçin)
              </label>
            ...
            <div className="flex flex-wrap gap-2 max-h-44 sm:max-h-52 overflow-y-auto pr-1 no-scrollbar">
              {allGenres.map(genre => {
            ...
            {Object.keys(genreFilters).length > 0 && (
              <div className="flex flex-wrap items-center gap-2 pt-1 text-xs text-gray-400">
                <span>Aktif Tür & Etiket Seçimleri:</span>
