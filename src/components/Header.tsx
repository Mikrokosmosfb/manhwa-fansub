...
export const Header: React.FC = () => {
  ...
  return (
    <header className="header-preserve-white bg-gradient-to-r from-purple-900 via-purple-950 to-indigo-950 text-white shadow-xl sticky top-0 z-50 border-b border-purple-500/20">
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 gap-2">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            {/* Mobile menu hamburger button - Clean, crisp icon without text */}
            <button
              onClick={() => {
                setIsMobileMenuOpen(!isMobileMenuOpen);
                setIsSearchOpen(false);
              }}
              className="p-2 rounded-xl text-purple-200 hover:text-white bg-purple-950/80 hover:bg-purple-900/90 border border-purple-500/30 hover:border-purple-400/60 transition-all flex items-center justify-center md:hidden shadow-md active:scale-95 flex-shrink-0"
              aria-label="Menü"
              title={isMobileMenuOpen ? "Menüyü Kapat" : "Menüyü Aç"}
            >
              {isMobileMenuOpen ? <X size={20} className="text-purple-200" /> : <Menu size={20} className="text-purple-200" />}
            </button>

            {/* Logo - ALWAYS FULL & UNTRUNCATED WITH TRANSPARENT LOGO */}
            <button
              onClick={() => setView({ type: 'home' })}
              className="flex items-center gap-2 sm:gap-2.5 text-left group flex-shrink-0 cursor-pointer bg-transparent border-0 p-0"
              title="Ana Sayfa"
            >
              <div className="relative flex items-center justify-center transition-transform duration-300 group-hover:scale-110 flex-shrink-0">
                {siteBranding.logoUrl && siteBranding.logoUrl.trim() ? (
                  <img
                    ...
                    width="40"
                    height="40"
                    className="w-9 h-9 sm:w-10 sm:h-10 object-contain drop-shadow-[0_0_12px_rgba(236,72,153,0.6)] group-hover:drop-shadow-[0_0_20px_rgba(56,189,248,0.9)] transition-all duration-300 bg-transparent"
                    onError={(e) => {
                      // Fallback to SaturnIcon if image load fails
                  ...
                  <SaturnIcon size={38} className="drop-shadow-[0_0_12px_rgba(236,72,153,0.6)] group-hover:drop-shadow-[0_0_20px_rgba(56,189,248,0.9)] transition-all duration-300" />
                )}
          ...
          {/* Desktop Central Nav Links with Icons & Active States */}
          <nav className="hidden md:flex items-center gap-1 lg:gap-1.5 text-xs lg:text-sm font-semibold bg-purple-950/40 p-1 rounded-2xl border border-purple-500/20 backdrop-blur-md">
            <button
              onClick={() => setView({ type: 'home' })}
            ...
            >
              <Home size={15} className={`transition-transform duration-200 group-hover:scale-110 ${view.type === 'home' ? 'text-white' : 'text-purple-300'}`} />
              <span>Ana Sayfa</span>
            </button>
            ...
            <button
              ...
              className={`group flex items-center gap-1.5 lg:gap-2 px-3 py-1.5 lg:px-3.5 lg:py-1.5 rounded-xl transition-all duration-200 active:scale-95 ${
                view.type === 'series-list'
                  ? 'bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-bold shadow-[0_0_12px_rgba(168,85,247,0.4)] border border-purple-400/40'
                  : 'text-purple-200/90 hover:text-white hover:bg-purple-800/40 hover:border-purple-500/20 border border-transparent'
              }`}
            >
              <BookOpen size={15} className={`transition-transform duration-200 group-hover:scale-110 ${view.type === 'series-list' ? 'text-white' : 'text-purple-300'}`} />
              <span>Seriler</span>
            </button>
            ...
            >
              <Bookmark size={15} className={`transition-transform duration-200 group-hover:scale-110 ${view.type === 'library' ? 'text-white' : 'text-purple-300'}`} />
              <span>Kütüphane</span>
            </button>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
            ...
            {/* Notification Bell Action Button with Dropdown on desktop & direct page navigation on mobile */}
            <div className="relative hidden md:block" ref={notificationButtonRef}>
              <button
                onClick={() => {
      ...
      {/* Mobile Nav Drawer */}
      {isMobileMenuOpen && (
        <div className="md:hidden bg-gray-950 border-t border-purple-800/80 px-4 pt-3 pb-5 space-y-2 shadow-2xl">
          {/* PWA App Download Banner in Mobile Drawer */}
          <div className="mb-2">
          ...
          <button
            onClick={() => { setView({ type: 'social-media' }); setIsMobileMenuOpen(false); }}
            className="w-full text-left px-3.5 py-2 rounded-xl text-sm font-extrabold text-pink-300 bg-pink-950/40 border border-pink-500/30 hover:bg-pink-900/50 flex items-center gap-2"
          >
            <Share2 size={16} className="text-pink-400" /> Sosyal Medya
          </button>
