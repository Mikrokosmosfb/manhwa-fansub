...
export const AdminModal: React.FC<AdminModalProps> = ({
  ...
  const [shopCategoryFilter, setShopCategoryFilter] = useState<'all' | 'theme_photo' | 'theme_aura' | 'frame' | 'badge' | 'emoji_pack'>('all');
  ...
  const [isShopR2Uploading, setIsShopR2Uploading] = useState(false);
  const [shopR2UploadMessage, setShopR2UploadMessage] = useState<{ type: 'success' | 'error'; text: string } | null>(null);
...
const handleUploadShopImageToR2 = async (e: React.ChangeEvent<HTMLInputElement>, target: 'theme_bg' | 'shop_icon' | 'frame_img') => {
    const file = e.target.files?.[0];
    if (!file) return;
    ...
    try {
      ...
      reader.onload = async () => {
        ...
        if (data.success && data.url) {
          if (target === 'theme_bg' && editingThemeStyle) {
            setEditingThemeStyle({ ...editingThemeStyle, cardBgImageUrl: data.url });
          } else if (target === 'frame_img' && editingShopItem) {
            setEditingShopItem({ ...editingShopItem, frameImageUrl: data.url });
          } else if (target === 'shop_icon' && editingShopItem) {
            setEditingShopItem({ ...editingShopItem, icon: data.url });
          }
          setShopR2UploadMessage({
            type: 'success',
            text: `🚀 Görsel Cloudflare R2 bucket deposuna başarıyla yüklendi!`
          });
        } else {
          setShopR2UploadMessage({ type: 'error', text: data.message || 'Görsel yüklemesi başarısız.' });
        }
      };
      reader.readAsDataURL(file);
    } catch (err: any) {
      setIsShopR2Uploading(false);
      setShopR2UploadMessage({ type: 'error', text: 'Dosya okuma hatası: ' + err.message });
    }
  };
  ...
  const handleCreateNewFrame = () => {
    const newId = `frame_custom_${Date.now()}`;
    const newItem: ShopItem = {
      id: newId,
      name: 'Yeni Profil Çerçevesi',
      category: 'frame',
      price: 250,
      description: 'Yönetici tarafından eklenen ışıltılı profil avatar çerçevesi.',
      icon: '🖼️',
      rarity: 'Destansı',
      frameStyle: '',
      frameImageUrl: '',
      frameScale: 145,
      frameOffsetY: 0,
      frameHideBorder: true
    };
    addShopItemAndStyle(newItem);
    handleOpenShopEdit(newItem);
  };

  const handleCreateNewBadge = () => {
    const newId = `badge_custom_${Date.now()}`;
    const newItem: ShopItem = {
      id: newId,
      name: 'Yeni Özel Unvan',
      category: 'badge',
      price: 150,
      description: 'Yönetici tarafından eklenen özel profil unvanı.',
      icon: '⭐',
      rarity: 'Destansı'
    };
    addShopItemAndStyle(newItem);
  ...
  const handleExecuteBloggerImport = async () => {
    if (bloggerImportMode === 'auto-smart') {
      ...
      const res = await addBatchSeries(batchToSave, { notifyNewSeries: true });
  ...
  const [notifyNewSeriesAll, setNotifyNewSeriesAll] = useState(true);
  const [notifyNewSeriesCustomMessage, setNotifyNewSeriesCustomMessage] = useState('');
  ...
  const handleCreateSeries = (e: React.FormEvent) => {
    ...
    const newSeries: Series = {
      ...
      heroImage: bannerImage.trim() || undefined,
      type,
      status,
    ...
    addOrUpdateSeries(newSeries, {
      notifyNewSeries: notifyNewSeriesAll,
      customMessage: notifyNewSeriesCustomMessage.trim() || undefined
    });
    alert(`"${newSeries.title}" serisi başarıyla eklendi!${notifyNewSeriesAll ? ' Tüm kullanıcılara yeni seri bildirimi gönderildi.' : ''}`);
    ...
    setIsNew(false);
    setIsGuncel(false);
    setNotifyNewSeriesCustomMessage('');
  ...
  const handleSaveEditSeries = (e: React.FormEvent) => {
    ...
    const updatedSeries: Series = {
      ...editingSeries,
      updatedAt: new Date().toISOString(),
      bannerImage: editingSeries.bannerImage?.trim() || undefined,
      heroImage: editingSeries.bannerImage?.trim() || editingSeries.heroImage?.trim() || undefined,
      customBadges: parsedBadges.length > 0 ? parsedBadges : undefined
    };
  ...
  if (!isAdminLoggedIn && !isSuperAdmin) {
    return (
      <div className="max-w-md mx-auto my-12 px-4 py-8">
        <div className="bg-gray-900 border-2 border-purple-500/40 rounded-3xl p-4 sm:p-6 lg:p-8 shadow-2xl space-y-6 text-center relative overflow-hidden">
  ...
  return (
    <div className={`${hideHeader ? 'w-full space-y-5' : 'max-w-5xl mx-auto px-4 py-8 space-y-6'}`}>
      ...
      {/* Add Series Form */}
      {activeTab === 'add-series' && (
        <form onSubmit={handleCreateSeries} className="bg-gray-900/95 border border-purple-500/20 rounded-3xl p-4 sm:p-6 lg:p-8 shadow-xl space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
            ...
            {/* Kapak Resmi (URL veya Bilgisayardan Yükleme) */}
            <div className="sm:col-span-2">
              <ImageUploadField
                ...
                required
                aspectRatio="cover"
                placeholder="https://... veya bilgisayarınızdan resim seçin / sürükleyin"
                helpText="Kapak görseli seri listelerinde ve kartlarda dikey (2:3 / 3:4) formatta görüntülenir."
              />
            </div>

            {/* Arka Plan / Banner Resmi (URL veya Bilgisayardan Yükleme) */}
            <div className="sm:col-span-2">
              <ImageUploadField
                label="Arka Plan / Banner Resmi (Opsiyonel)"
                value={bannerImage}
                onChange={setBannerImage}
                aspectRatio="banner"
                placeholder="https://... veya bilgisayarınızdan geniş banner seçin"
                helpText="Seri detay sayfasının üst arka planında ve anasayfa vitrininde geniş formatta (16:9) gösterilir."
              />
            </div>
          ...
          <div>
            <label className="block text-purple-300 font-bold text-xs mb-1">
              Seri Notu / Duyuru (Opsiyonel)
            </label>
            <input
              type="text"
              placeholder="Örn: Yeni bölümler her hafta Pazartesi 18:00'de yüklenecektir."
              value={notice}
          ...
          <div className="bg-gradient-to-r from-purple-950/70 via-gray-900 to-indigo-950/70 border border-purple-500/40 rounded-2xl p-4 space-y-3 shadow-lg">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-xl bg-purple-600/30 border border-purple-400/50 flex items-center justify-center text-purple-300 shrink-0">
                  <Bell size={18} />
                </div>
                <div>
                  <p className="text-xs font-black text-white flex items-center gap-2">
                    Tüm Kullanıcılara "Yeni Seri" Bildirimi Gönder
                  ...
                  <p className="text-[11px] text-gray-400">
                    Seri eklendiğinde tüm ziyaretçilerin bildirim kutusuna ve ekrana anında canlı bildirim düşer.
                  </p>
                </div>
              </div>
              <input
                type="checkbox"
                checked={notifyNewSeriesAll}
                onChange={e => setNotifyNewSeriesAll(e.target.checked)}
                className="w-5 h-5 accent-purple-500 rounded cursor-pointer"
              />
            </div>
            {notifyNewSeriesAll && (
              <div className="pt-2 border-t border-purple-900/40">
                <label className="block text-[11px] font-bold text-purple-300 mb-1">
                  Özel Bildirim Mesajı (Opsiyonel - Boş bırakılırsa seri özetiyle otomatik oluşturulur):
                </label>
                <input
                  type="text"
                  placeholder="Örn: Yeni manhwa serimiz güncel bölümleriyle sitemizde! Hemen ilk bölümü okuyun."
                  value={notifyNewSeriesCustomMessage}
                  onChange={e => setNotifyNewSeriesCustomMessage(e.target.value)}
                  className="w-full bg-gray-950 border border-purple-500/30 text-white text-xs rounded-xl px-3.5 py-2.5 focus:outline-none placeholder-gray-600 focus:border-purple-400 transition"
                />
              </div>
            )}
          </div>
          ...
          >
            <Save size={18} />
            Seriyi Kaydet {notifyNewSeriesAll ? '& Bildirimi Yayınla' : ''}
          </button>
        </form>
      )}
      ...
      {/* Add Chapter Form */}
      {activeTab === 'add-chapter' && (
        <form onSubmit={handleAddChapter} className="bg-gray-900/95 border border-purple-500/20 rounded-3xl p-4 sm:p-6 lg:p-8 shadow-xl space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
            <div>
          ...
          {editingSeries && (
            <form onSubmit={handleSaveEditSeries} className="bg-purple-950/90 border-2 border-purple-500 rounded-3xl p-6 shadow-2xl space-y-4 animate-fadeIn">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                ...
                {/* Series Type */}
                <div>
                  <label className="block text-purple-300 font-bold mb-1">Seri Formatı (Tipi)</label>
                  <select
                    value={editingSeries.type || 'Manhwa'}
                    onChange={e => setEditingSeries({ ...editingSeries, type: e.target.value as SeriesType })}
                    className="w-full bg-gray-950 border border-purple-500/30 text-white rounded-xl p-2.5 focus:outline-none font-semibold"
                  >
                    <option value="Manhwa">Manhwa (Webtoon / Kore)</option>
                    <option value="Web Novel">Web Novel (Metin Roman)</option>
                    <option value="Manga">Manga (Japon Çizgi Romanı)</option>
                    <option value="Manhua">Manhua (Çin Çizgi Romanı)</option>
                    <option value="Webtoon">Webtoon</option>
                    <option value="One Shot">One Shot (Tek Bölümlük)</option>
                  </select>
                </div>

                {/* Rating */}
                <div>
                  <label className="block text-purple-300 font-bold mb-1">Puan / Değerlendirme (1 - 10)</label>
                  <input
                    type="number"
                    step="0.1"
                    min="1"
                    max="10"
                    placeholder="9.5"
                    value={editingSeries.rating ?? 9.5}
                    onChange={e => setEditingSeries({ ...editingSeries, rating: parseFloat(e.target.value) || 9.5 })}
                    className="w-full bg-gray-950 border border-purple-500/30 text-amber-300 font-bold rounded-xl p-2.5 focus:outline-none"
                  />
                </div>

                {/* Cover Image (URL veya Bilgisayardan Yükleme) */}
                <div className="sm:col-span-2">
                  <ImageUploadField
                    label="Kapak Resmi"
                    value={editingSeries.coverImage || ''}
                    onChange={val => setEditingSeries({ ...editingSeries, coverImage: val })}
                    required
                    aspectRatio="cover"
                    placeholder="https://... veya bilgisayardan kapak resmi seçin"
                    helpText="Kapak resmi dikey (2:3 veya 3:4 oranında) en iyi sonucu verir."
                  />
                </div>

                {/* Banner / Header Image (URL veya Bilgisayardan Yükleme) */}
                <div className="sm:col-span-2">
                  <ImageUploadField
                    label="Arka Plan / Banner Resmi (Opsiyonel)"
                    value={editingSeries.bannerImage || ''}
                    onChange={val => setEditingSeries({ ...editingSeries, bannerImage: val })}
                    aspectRatio="banner"
                    placeholder="https://... veya bilgisayardan geniş banner seçin"
                    helpText="Geniş formatlı arka plan görseli (16:9 önerilir)."
                  />
                </div>
                ...
                <div className="sm:col-span-2">
                  ...
                  <textarea
                    rows={4}
                    placeholder="Seri konusunu ve açıklamasını buraya yazın..."
                ...
                <div className="sm:col-span-2">
                  <label className="block text-purple-300 font-bold mb-2">Türler (Kategoriler)</label>
                  <div className="flex flex-wrap gap-1.5 max-h-36 overflow-y-auto pr-1">
                    {GENRE_LIST.map(g => {
                      const isSel = (editingSeries.genres || []).includes(g);
                      return (
                        <button
                          key={g}
                          type="button"
                          onClick={() => {
                            const cur = editingSeries.genres || [];
                            const next = cur.includes(g) ? cur.filter(x => x !== g) : [...cur, g];
                            setEditingSeries({ ...editingSeries, genres: next });
                          }}
                          className={`px-2.5 py-1 rounded-lg text-xs font-semibold transition ${
                            isSel ? 'bg-purple-600 text-white shadow-sm' : 'bg-gray-950 text-gray-400 border border-gray-800 hover:border-purple-500/50'
                          }`}
                        >
                          {g}
                        </button>
              ...
              {/* Feature Badges Grid (Güncel, Sıcak, Yeni, 18+) */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {/* Güncel Seri Etiketi */}
                <div className="bg-cyan-950/40 border border-cyan-800/50 rounded-2xl p-3 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                  ...
                  <input
                    type="checkbox"
                    checked={editingSeries.isGuncel || false}
                    onChange={e => setEditingSeries({ ...editingSeries, isGuncel: e.target.checked })}
                    className="w-5 h-5 accent-cyan-500 rounded cursor-pointer"
                  />
                </div>

                {/* Sıcak / Popüler Etiketi */}
                <div className="bg-red-950/40 border border-red-800/50 rounded-2xl p-3 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="bg-red-600 text-white font-black text-xs px-2 py-0.5 rounded-md flex items-center gap-1">
                      <Flame size={12} /> SICAK
                  ...
                  <input
                    type="checkbox"
                    checked={editingSeries.isHot || false}
                    onChange={e => setEditingSeries({ ...editingSeries, isHot: e.target.checked })}
                    className="w-5 h-5 accent-red-500 rounded cursor-pointer"
                  />
                </div>
                ...
                <div className="bg-purple-950/40 border border-purple-800/50 rounded-2xl p-3 flex items-center justify-between">
                  ...
                  <input
                    type="checkbox"
                    checked={editingSeries.isNew || false}
                    onChange={e => setEditingSeries({ ...editingSeries, isNew: e.target.checked })}
                    className="w-5 h-5 accent-purple-500 rounded cursor-pointer"
                  />
                </div>
      ...
      {activeTab === 'blogger-import' && (
        <div className="bg-gray-900/95 border border-orange-500/30 rounded-3xl p-4 sm:p-6 lg:p-8 shadow-xl space-y-6 animate-fadeIn">
      ...
      {/* Cloudflare D1 Tab */}
      {activeTab === 'cloudflare-d1' && (
        <div className="bg-gray-900/95 border border-cyan-500/30 rounded-3xl p-4 sm:p-6 lg:p-8 shadow-xl space-y-6 animate-fadeIn">
      ...
      {activeTab === 'shop-management' && (
        <div className="bg-gray-900/95 border border-amber-500/30 rounded-3xl p-4 sm:p-6 lg:p-8 shadow-xl space-y-6 animate-fadeIn">
          {/* Header */}
          <div className="flex items-center justify-between flex-wrap gap-4 border-b border-gray-800 pb-4">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <button
                ...
                className="bg-gradient-to-r from-amber-500 to-yellow-500 hover:from-amber-400 hover:to-yellow-400 text-black font-extrabold text-xs px-3.5 py-2.5 rounded-xl shadow-lg flex items-center gap-1.5 transition transform hover:scale-105"
              >
                <PlusCircle size={15} />
                + Tema Ekle
              </button>
              <button
                ...
                className="bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-400 hover:to-blue-400 text-white font-extrabold text-xs px-3.5 py-2.5 rounded-xl shadow-lg flex items-center gap-1.5 transition transform hover:scale-105"
              >
                <Sparkles size={15} />
                + Çerçeve Ekle
              </button>
              <button
                type="button"
                onClick={handleCreateNewBadge}
                className="bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-400 hover:to-pink-400 text-white font-extrabold text-xs px-3.5 py-2.5 rounded-xl shadow-lg flex items-center gap-1.5 transition transform hover:scale-105"
              >
                <Award size={15} />
                + Unvan Ekle
              </button>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-gray-950/80 p-3 rounded-2xl border border-amber-500/20">
            ...
            <div className="flex items-center gap-1.5 overflow-x-auto w-full sm:w-auto pb-1 sm:pb-0">
              ...
              <button
                type="button"
                onClick={() => setShopCategoryFilter('frame')}
                className={`px-3 py-1.5 rounded-lg text-xs font-extrabold whitespace-nowrap transition ${
                  shopCategoryFilter === 'frame'
                    ? 'bg-cyan-500 text-black'
                    : 'bg-gray-900 text-cyan-300 hover:text-white'
                }`}
              >
                💎 Çerçeveler ({shopItems.filter(i => i.category === 'frame').length})
              </button>
              <button
                type="button"
                ...
                return (
                  ...
                  >
                    <div>
                      ...
                      {/* Preview for Frames */}
                      {item.category === 'frame' && (
                        <div className="relative rounded-2xl overflow-hidden p-3.5 border border-cyan-500/30 bg-gray-900/90 my-2 flex items-center gap-3.5">
                          <UserAvatar
                          ...
                          <div className="space-y-0.5">
                            <span className="text-xs font-bold text-cyan-300 flex items-center gap-1">
                              <Sparkles size={12} /> Profil Çerçevesi Canlı Önizleme
                            </span>
                            <span className="text-[10px] text-gray-400 block">
                              {item.frameImageUrl ? `🖼️ Şeffaf PNG Katmanı (Ölçek: %${item.frameScale || 140})` : '✨ CSS Işıltı Efekti'}
                            </span>
                          </div>
                        </div>
                      )}
                      ...
                      {item.category === 'theme' && style && (
                        <div className="relative rounded-xl overflow-hidden p-3 border border-gray-800 bg-gray-900 my-2 min-h-[100px] flex flex-col justify-end">
                          ...
                          <div className="relative z-10 space-y-1.5">
                            <div className={`flex items-center gap-2 w-fit ${
                              style.cardBgImageUrl ? 'bg-black/75 px-2.5 py-1 rounded-lg border border-white/10' : ''
                            }`}>
                            ...
                            <p className={`text-[11px] text-gray-200 ${
                              style.cardBgImageUrl ? 'bg-black/75 px-2.5 py-1.5 rounded-lg border border-white/10' : ''
                            }`}>
        ...
        <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-md flex items-center justify-center p-4 overflow-y-auto animate-fadeIn">
          <div className="relative bg-gray-900 border-2 border-amber-500/50 rounded-3xl max-w-2xl w-full p-6 shadow-2xl space-y-5 my-8 max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
              ...
              <div>
                <label className="block text-amber-300 font-bold mb-1">Ürün Simgesi / Görseli *</label>
                <div className="flex items-center gap-2">
                  {(editingShopItem.icon?.startsWith('http') || editingShopItem.icon?.startsWith('/')) && (
                  ...
                  <input
                    ...
                    className="w-full bg-gray-950 border border-gray-700 text-white rounded-xl p-2.5 focus:outline-none focus:border-amber-400"
                  />
                  <label className="relative bg-gradient-to-r from-amber-500 to-yellow-500 hover:from-amber-400 hover:to-yellow-400 text-black font-extrabold text-xs px-3 py-2.5 rounded-xl cursor-pointer flex items-center gap-1.5 transition whitespace-nowrap shadow-md">
                    <UploadCloud size={15} />
                    <span>R2'ye Yükle</span>
                    <input
                      type="file"
                      accept="image/*"
                      onChange={e => handleUploadShopImageToR2(e, 'shop_icon')}
              ...
              <div>
                <label className="block text-amber-300 font-bold mb-1">Kategori *</label>
                <select
                  value={editingShopItem.category}
                  onChange={e => {
                    const newCat = e.target.value as any;
                    setEditingShopItem({ ...editingShopItem, category: newCat });
                  }}
                  className="w-full bg-gray-950 border border-gray-700 text-white rounded-xl p-2.5 focus:outline-none focus:border-amber-400"
                >
                  <option value="frame">💎 Profil Çerçevesi (Frame)</option>
                  <option value="theme">🎨 Yorum & Profil Teması (Theme)</option>
                  <option value="badge">⭐ Özel Unvan / Rozet (Badge)</option>
                  <option value="emoji_pack">😊 Emoji Paketi</option>
                </select>
              </div>
            ...
            {/* Profile Frame Specific Settings */}
            {editingShopItem.category === 'frame' && (
              <div className="space-y-4 pt-3 border-t border-cyan-800/40">
                <div className="flex items-center justify-between">
                  <h4 className="text-xs font-black text-cyan-300 uppercase tracking-wider flex items-center gap-1.5">
                    <Sparkles size={14} className="text-cyan-400" /> Profil Çerçevesi Görseli & Hassas Uyum Ayarları
                  </h4>
                  <span className="text-[10px] text-cyan-400 bg-cyan-950/80 px-2 py-0.5 rounded-full border border-cyan-800">
                    Avatar Üzeri Şeffaf Katman
                  </span>
                </div>

                {/* Cloudflare R2 Çerçeve Yükleme */}
                <div className="bg-cyan-950/30 border border-cyan-500/40 rounded-2xl p-3.5 space-y-2 shadow-inner">
                  <div className="flex items-center justify-between">
                    <label className="text-xs font-extrabold text-cyan-300 flex items-center gap-1.5">
                      <UploadCloud size={16} className="text-cyan-400 animate-bounce" />
                      💻 Şeffaf Çerçeve Yükleyin (PNG, Hareketli WebP, GIF veya Şeffaf WebM)
                    </label>
                    {isShopR2Uploading && (
                      <span className="text-[11px] font-bold text-cyan-300 animate-pulse flex items-center gap-1 bg-black/40 px-2 py-0.5 rounded-lg border border-cyan-500/30">
                        <Loader2 size={13} className="animate-spin text-cyan-400" /> R2'ye Yükleniyor...
                      </span>
                    )}
                  </div>

                  <div className="relative border-2 border-dashed border-cyan-500/50 hover:border-cyan-400 bg-gray-950/80 hover:bg-black rounded-2xl p-4 text-center transition cursor-pointer flex flex-col items-center justify-center gap-1.5 group">
                    <input
                      type="file"
                      accept="image/*,video/webm,video/mp4"
                      disabled={isShopR2Uploading}
                      onChange={e => handleUploadShopImageToR2(e, 'frame_img')}
                      className="absolute inset-0 opacity-0 cursor-pointer w-full h-full z-10"
                    />
                    <UploadCloud size={28} className="text-cyan-400 group-hover:scale-110 transition transform" />
                    <div>
                      <p className="text-xs font-black text-cyan-200">
                        Bilgisayarınızdan Şeffaf PNG, Hareketli WebP, GIF veya WebM Video Seçin
                      </p>
                      <p className="text-[10px] text-gray-400 mt-0.5">
                        Avatarın etrafında dönecek şeffaf görsel, animasyon veya ultra hafif WebM video katmanı
                      </p>
                    </div>
                  </div>
                </div>

                {/* Manuel Çerçeve Görsel Linki */}
                <div>
                  <label className="block text-xs font-bold text-cyan-200 mb-1">
                    🔗 Veya Şeffaf Çerçeve Linki (PNG / GIF / Hareketli WebP / WebM Video URL)
                  </label>
                  <div className="flex items-center gap-2">
                    <input
                      type="text"
                      placeholder="https://... (.png, .webp, .gif, .webm veya R2 linki)"
                      value={editingShopItem.frameImageUrl || ''}
                      onChange={e => setEditingShopItem({ ...editingShopItem, frameImageUrl: e.target.value })}
                      className="w-full bg-gray-950 border border-cyan-500/40 text-cyan-200 text-xs rounded-xl p-2.5 focus:outline-none focus:ring-1 focus:ring-cyan-400 font-mono"
                    />
                    {editingShopItem.frameImageUrl && (
                      <button
                        type="button"
                        onClick={() => setEditingShopItem({ ...editingShopItem, frameImageUrl: '' })}
                        className="bg-gray-800 text-gray-400 hover:text-red-400 p-2.5 rounded-xl text-xs border border-gray-700 transition"
                ...
                {/* Çerçeve Boyut Ölçeği (Scale) & Dikey Konum (Offset) */}
                <div className="bg-gray-950/90 border border-cyan-500/30 rounded-2xl p-4 space-y-3.5">
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <label className="text-xs font-black text-cyan-200 flex items-center gap-1.5">
                        📐 Çerçeve Boyut Oranı (Ölçek): <span className="text-cyan-400 font-mono text-sm font-black">%{editingShopItem.frameScale ?? 145}</span>
                      </label>
                      <span className="text-[10px] text-gray-400">
                        (Avatarın dışına tam oturması için %135-%160 önerilir)
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                      <input
                        ...
                        max="200"
                        step="1"
                        value={editingShopItem.frameScale ?? 145}
                        onChange={e => setEditingShopItem({ ...editingShopItem, frameScale: Number(e.target.value) })}
                      ...
                      <input
                        ...
                        value={editingShopItem.frameScale ?? 145}
                        onChange={e => setEditingShopItem({ ...editingShopItem, frameScale: Number(e.target.value) })}
                        className="w-16 bg-gray-900 border border-cyan-500/40 text-cyan-300 text-center text-xs font-bold rounded-lg p-1.5 focus:outline-none"
                      />
                    </div>

                    {/* Hızlı Boyut Butonları */}
                    <div className="flex items-center gap-1.5 flex-wrap pt-1">
                      <span className="text-[10px] font-bold text-gray-400">Hızlı Ölçek:</span>
                      {[
                        { label: 'Dar (%125)', val: 125 },
                        { label: 'Normal (%135)', val: 135 },
                        { label: 'Taçlı/Süslü (%145)', val: 145 },
                        { label: 'Geniş (%155)', val: 155 },
                        { label: 'Ekstra Geniş (%170)', val: 170 },
                      ].map(p => (
                        <button
                          key={p.val}
                          type="button"
                          onClick={() => setEditingShopItem({ ...editingShopItem, frameScale: p.val })}
                          className={`px-2.5 py-1 rounded-lg text-[10px] font-bold transition ${
                            (editingShopItem.frameScale ?? 145) === p.val
                              ? 'bg-cyan-500 text-black font-extrabold shadow'
                              : 'bg-gray-900 hover:bg-gray-800 text-cyan-300 border border-cyan-800/60'
                          }`}
                        >
                          {p.label}
                        </button>
                  ...
                  <div className="space-y-2 pt-2 border-t border-gray-800/80">
                    <div className="flex items-center justify-between">
                      <label className="text-xs font-black text-cyan-200 flex items-center gap-1.5">
                        ↕️ Dikey Hizalama (Y-Offset): <span className="text-cyan-400 font-mono text-sm font-black">{editingShopItem.frameOffsetY ?? 0}%</span>
                      </label>
                      <span className="text-[10px] text-gray-400">
                        (Taç veya taban süsleri için yukarı (-) / aşağı (+))
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                      <input
                        ...
                        max="30"
                        step="1"
                        value={editingShopItem.frameOffsetY ?? 0}
                        onChange={e => setEditingShopItem({ ...editingShopItem, frameOffsetY: Number(e.target.value) })}
                        className="w-full h-2 bg-gray-800 rounded-lg appearance-none cursor-pointer accent-cyan-400"
                      />
                      <button
                        type="button"
                        onClick={() => setEditingShopItem({ ...editingShopItem, frameOffsetY: 0 })}
                        className="px-2.5 py-1 bg-gray-900 hover:bg-gray-800 text-gray-300 text-[10px] font-bold rounded-lg border border-gray-700 whitespace-nowrap"
                      >
                        Sıfırla (0%)
                      </button>
                  ...
                  {/* Arka Plan Halka / Kenarlık Ayarı */}
                  <div className="pt-2 border-t border-gray-800/80 flex items-center justify-between">
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={editingShopItem.frameHideBorder ?? true}
                        onChange={e => setEditingShopItem({ ...editingShopItem, frameHideBorder: e.target.checked })}
                        className="w-4 h-4 rounded text-cyan-500 bg-gray-900 border-gray-700 focus:ring-0 cursor-pointer"
                      />
                      <span className="text-xs font-bold text-cyan-200">
                        ⭕ PNG Çerçeve Arkasındaki Kalın CSS Halkasını Gizle
                      </span>
                    </label>
                    <span className="text-[10px] text-emerald-400 font-semibold bg-emerald-950/60 px-2 py-0.5 rounded border border-emerald-800">
                      Önerilir (Temiz Görünüm)
                    </span>
                  </div>
                </div>

                {/* İsteğe Bağlı Ekstra CSS Işıltı Stili */}
                <div className="space-y-2">
                  <label className="block text-xs font-bold text-gray-300">
                    ✨ (İsteğe Bağlı) Ekstra CSS Işıltı / Halka Efekti (Tailwind)
                  </label>
                  <input
                    type="text"
                    placeholder="Boş bırakabilir veya isteğe bağlı 'shadow-[0_0_20px_rgba(6,182,212,0.8)]' ekleyebilirsiniz"
                    value={editingShopItem.frameStyle || ''}
                    onChange={e => setEditingShopItem({ ...editingShopItem, frameStyle: e.target.value })}
                    className="w-full bg-gray-950 border border-gray-700 text-cyan-300 text-xs rounded-xl p-2.5 focus:outline-none focus:border-cyan-400 font-mono"
                  />

                  {/* Hazır Halka Preset Butonları */}
                  <div className="space-y-1 pt-1">
                    <div className="flex items-center justify-between">
                      <span className="text-[11px] font-bold text-gray-400">⚡ Hızlı CSS Efektleri:</span>
                      {editingShopItem.frameStyle && (
                        <button
                          type="button"
                          onClick={() => setEditingShopItem({ ...editingShopItem, frameStyle: '' })}
                          className="text-[10px] text-red-400 hover:underline"
                        >
                          CSS Efektini Temizle
                        </button>
                      )}
                    </div>
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <button
                        type="button"
                        onClick={() => setEditingShopItem({
                          ...editingShopItem,
                          frameStyle: 'shadow-[0_0_25px_rgba(6,182,212,0.8)]'
                        })}
                        className="px-2.5 py-1 bg-cyan-950 hover:bg-cyan-900 border border-cyan-500/50 text-cyan-300 rounded-lg text-[10px] font-bold transition"
                      >
                        Cyan Işıltı
                      </button>
                      <button
                        ...
                        onClick={() => setEditingShopItem({
                          ...editingShopItem,
                          frameStyle: 'shadow-[0_0_25px_rgba(245,158,11,0.9)] animate-pulse'
                        })}
                        className="px-2.5 py-1 bg-amber-950 hover:bg-amber-900 border border-amber-500/50 text-amber-300 rounded-lg text-[10px] font-bold transition"
                      >
                        Altın Işıltı
                      </button>
                      <button
                        ...
                        onClick={() => setEditingShopItem({
                          ...editingShopItem,
                          frameStyle: 'shadow-[0_0_25px_rgba(168,85,247,0.9)]'
                        })}
                        className="px-2.5 py-1 bg-purple-950 hover:bg-purple-900 border border-purple-500/50 text-purple-300 rounded-lg text-[10px] font-bold transition"
                      >
                        Kozmik Mor
                      </button>
                      <button
                        ...
                        onClick={() => setEditingShopItem({
                          ...editingShopItem,
                          frameStyle: 'shadow-[0_0_25px_rgba(244,63,94,0.9)]'
                        })}
                        className="px-2.5 py-1 bg-rose-950 hover:bg-rose-900 border border-rose-500/50 text-rose-300 rounded-lg text-[10px] font-bold transition"
                      >
                        Yakut Aşk
                      </button>
                    </div>
                ...
                <div className="space-y-2 pt-2">
                  <div className="flex items-center justify-between">
                    <label className="text-xs font-black text-cyan-300 flex items-center gap-1">
                      <Sparkles size={14} className="text-cyan-400" /> Canlı Avatar Çerçevesi Önizlemesi (Live Preview):
                    </label>
                    <span className="text-[10px] text-cyan-300 font-bold bg-cyan-950/80 px-2 py-0.5 rounded border border-cyan-800">
                      Ölçek: %{editingShopItem.frameScale ?? 145}
                    </span>
                  </div>
                  <div className="bg-gray-950 border border-cyan-500/40 rounded-2xl p-5 flex items-center justify-around gap-4 flex-wrap shadow-2xl">
                    <div className="text-center space-y-1">
                      <UserAvatar
                        avatar="https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=100&auto=format&fit=crop&q=80"
                        name="Okuyucu"
                        frameId={editingShopItem.id}
                        customFrameStyle={editingShopItem.frameStyle}
                        customFrameImageUrl={editingShopItem.frameImageUrl}
                        customFrameScale={editingShopItem.frameScale}
                        customFrameOffsetY={editingShopItem.frameOffsetY}
                        customFrameOffsetX={editingShopItem.frameOffsetX}
                        customFrameHideBorder={editingShopItem.frameHideBorder}
                        size="xl"
                      />
                      <span className="text-[10px] font-extrabold text-cyan-200 block mt-1">Büyük (Profil)</span>
                    </div>
                    ...
                    <div className="text-center space-y-1">
                      <UserAvatar
                        ...
                        size="lg"
                      />
                      <span className="text-[10px] font-extrabold text-cyan-200 block mt-1">Orta (Yorum)</span>
                    </div>
                    ...
                    <div className="text-center space-y-1">
                      <UserAvatar
                        ...
                        size="md"
                      />
                      <span className="text-[10px] font-extrabold text-cyan-200 block mt-1">Küçük (Başlık)</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {editingShopItem.category === 'theme' && editingThemeStyle && (
              <div className="space-y-4 pt-3 border-t border-gray-800">
                ...
                {/* Cloudflare R2 Bilgisayardan Yükleme Alanı */}
                <div className="bg-amber-950/30 border border-amber-500/40 rounded-2xl p-3.5 space-y-2 shadow-inner">
                  <div className="flex items-center justify-between">
                    <label className="text-xs font-extrabold text-amber-300 flex items-center gap-1.5">
                      <UploadCloud size={16} className="text-amber-400 animate-bounce" />
                      💻 Bilgisayarınızdan Görsel Yükleyin (Cloudflare R2 Bucket Storage)
                    </label>
                    {isShopR2Uploading && (
                      <span className="text-[11px] font-bold text-amber-300 animate-pulse flex items-center gap-1 bg-black/40 px-2 py-0.5 rounded-lg border border-amber-500/30">
                        <Loader2 size={13} className="animate-spin text-amber-400" /> Cloudflare R2'ye Yükleniyor...
                      </span>
                    )}
                  </div>

                  <div className="relative border-2 border-dashed border-amber-500/50 hover:border-amber-400 bg-gray-950/80 hover:bg-black rounded-2xl p-4 text-center transition cursor-pointer flex flex-col items-center justify-center gap-1.5 group">
                    <input
                      type="file"
                      accept="image/*"
                      disabled={isShopR2Uploading}
                      onChange={e => handleUploadShopImageToR2(e, 'theme_bg')}
                      className="absolute inset-0 opacity-0 cursor-pointer w-full h-full z-10"
                    />
                    <UploadCloud size={28} className="text-amber-400 group-hover:scale-110 transition transform" />
                    <div>
                      <p className="text-xs font-black text-amber-200">
                        Bilgisayarınızdan İstediğiniz Görseli Seçin veya Buraya Sürükleyin
                      </p>
                      <p className="text-[10px] text-gray-400 mt-0.5">
                        PNG, JPG, WebP veya GIF — Yüklediğiniz görsel doğrudan Cloudflare R2 Object Storage'a kaydedilir
                      </p>
                    </div>
                  </div>
                  ...
                  {shopR2UploadMessage && (
                    <div className={`p-2.5 rounded-xl text-xs font-bold flex items-center gap-2 ${
                      shopR2UploadMessage.type === 'success'
                        ? 'bg-emerald-950/90 border border-emerald-500/50 text-emerald-300'
                        : 'bg-red-950/90 border border-red-500/50 text-red-300'
                    }`}>
                      <CheckCircle2 size={15} />
                      <span>{shopR2UploadMessage.text}</span>
                    </div>
                  )}
                </div>

                {/* Manuel Arka Plan Görsel URL Input */}
                <div>
                  <label className="block text-xs font-bold text-gray-200 mb-1">
                    🔗 Veya Harici Görsel Linki (Image URL)
                  </label>
                  <div className="flex items-center gap-2">
                    <input
                      type="text"
                      placeholder="https://... veya Cloudflare R2 dosya linki"
                      value={editingThemeStyle.cardBgImageUrl || ''}
                      onChange={e => setEditingThemeStyle({ ...editingThemeStyle, cardBgImageUrl: e.target.value })}
                      className="w-full bg-gray-950 border border-amber-500/40 text-amber-200 text-xs rounded-xl p-2.5 focus:outline-none focus:ring-1 focus:ring-amber-400 font-mono"
                    />
                ...
                <div className="pt-4 mt-4 border-t border-gray-800 space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        const newDec = { id: 'pdec_' + Date.now(), imageUrl: '', top: '10%', right: '10%', width: '120px', zIndex: 20 };
                        setEditingThemeStyle({
                          ...editingThemeStyle,
                  ...
                  {(!editingThemeStyle.profileDecorations || editingThemeStyle.profileDecorations.length === 0) && (
                     <p className="text-xs text-gray-400 italic">Henüz profil için bir dekorasyon eklenmemiş. Profil başlığının üstünde veya yanlarında duracak dev chibiler ekleyebilirsiniz.</p>
                  )}
                    ...
                    {editingThemeStyle.profileDecorations?.map((dec, idx) => (
                      <div key={dec.id || idx} className="bg-gray-900 border border-gray-700 p-3 rounded-lg relative">
                        <button
                          type="button"
                          onClick={() => {
                            if (confirm('Bu dekorasyonu silmek istediğinize emin misiniz?')) {
                        ...
                        <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
                          <div>
                            ...
                            <input type="text" placeholder="örn: 10%" value={dec.top || ''} onChange={e => { const u = [...editingThemeStyle.profileDecorations!]; u[idx].top = e.target.value; setEditingThemeStyle({ ...editingThemeStyle, profileDecorations: u }); }} onBlur={e => { let v = e.target.value.trim(); if(v && !isNaN(Number(v))) { const u = [...editingThemeStyle.profileDecorations!]; u[idx].top = v + 'px'; setEditingThemeStyle({ ...editingThemeStyle, profileDecorations: u }); } }} className="w-full bg-gray-950 border border-gray-700 rounded p-1 text-white" />
                          </div>
                          <div>
                            <label className="text-[10px] text-gray-400 font-bold">Right</label>
                            <input type="text" placeholder="örn: 10%" value={dec.right || ''} onChange={e => { const u = [...editingThemeStyle.profileDecorations!]; u[idx].right = e.target.value; setEditingThemeStyle({ ...editingThemeStyle, profileDecorations: u }); }} onBlur={e => { let v = e.target.value.trim(); if(v && !isNaN(Number(v))) { const u = [...editingThemeStyle.profileDecorations!]; u[idx].right = v + 'px'; setEditingThemeStyle({ ...editingThemeStyle, profileDecorations: u }); } }} className="w-full bg-gray-950 border border-gray-700 rounded p-1 text-white" />
                          </div>
                          <div>
                            <label className="text-[10px] text-gray-400 font-bold">Bottom</label>
                            <input type="text" placeholder="örn: auto" value={dec.bottom || ''} onChange={e => { const u = [...editingThemeStyle.profileDecorations!]; u[idx].bottom = e.target.value; setEditingThemeStyle({ ...editingThemeStyle, profileDecorations: u }); }} onBlur={e => { let v = e.target.value.trim(); if(v && !isNaN(Number(v))) { const u = [...editingThemeStyle.profileDecorations!]; u[idx].bottom = v + 'px'; setEditingThemeStyle({ ...editingThemeStyle, profileDecorations: u }); } }} className="w-full bg-gray-950 border border-gray-700 rounded p-1 text-white" />
                          </div>
                          <div>
                            <label className="text-[10px] text-gray-400 font-bold">Left</label>
                            <input type="text" placeholder="örn: auto" value={dec.left || ''} onChange={e => { const u = [...editingThemeStyle.profileDecorations!]; u[idx].left = e.target.value; setEditingThemeStyle({ ...editingThemeStyle, profileDecorations: u }); }} onBlur={e => { let v = e.target.value.trim(); if(v && !isNaN(Number(v))) { const u = [...editingThemeStyle.profileDecorations!]; u[idx].left = v + 'px'; setEditingThemeStyle({ ...editingThemeStyle, profileDecorations: u }); } }} className="w-full bg-gray-950 border border-gray-700 rounded p-1 text-white" />
                          </div>
                          <div>
                            <label className="text-[10px] text-gray-400 font-bold">Width</label>
                            <input type="text" placeholder="örn: 120px" value={dec.width || ''} onChange={e => { const u = [...editingThemeStyle.profileDecorations!]; u[idx].width = e.target.value; setEditingThemeStyle({ ...editingThemeStyle, profileDecorations: u }); }} onBlur={e => { let v = e.target.value.trim(); if(v && !isNaN(Number(v))) { const u = [...editingThemeStyle.profileDecorations!]; u[idx].width = v + 'px'; setEditingThemeStyle({ ...editingThemeStyle, profileDecorations: u }); } }} className="w-full bg-gray-950 border border-gray-700 rounded p-1 text-white" />
                          </div>
                          <div>
                            <label className="text-[10px] text-gray-400 font-bold">Rotation</label>
                            <input type="text" placeholder="örn: -5deg" value={dec.rotation || ''} onChange={e => { const u = [...editingThemeStyle.profileDecorations!]; u[idx].rotation = e.target.value; setEditingThemeStyle({ ...editingThemeStyle, profileDecorations: u }); }} className="w-full bg-gray-950 border border-gray-700 rounded p-1 text-white" />
                          </div>
                          <div>
                            <label className="text-[10px] text-gray-400 font-bold">z-Index</label>
                            <input type="number" placeholder="örn: 20" value={dec.zIndex || ''} onChange={e => { const u = [...editingThemeStyle.profileDecorations!]; u[idx].zIndex = parseInt(e.target.value) || 0; setEditingThemeStyle({ ...editingThemeStyle, profileDecorations: u }); }} className="w-full bg-gray-950 border border-gray-700 rounded p-1 text-white" />
                          </div>
                        </div>
                      </div>
                    ))}
                      ...
                      <div key={dec.id} className="p-3 bg-gray-900 border border-purple-500/30 rounded-xl space-y-3">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
                          <div>
                            ...
                            <input type="text" placeholder="örn: -20px" value={dec.top || ''} onChange={e => { const u = [...editingThemeStyle.decorations!]; u[idx].top = e.target.value; setEditingThemeStyle({ ...editingThemeStyle, decorations: u }); }} onBlur={e => { let v = e.target.value.trim(); if(v && !isNaN(Number(v))) { const u = [...editingThemeStyle.decorations!]; u[idx].top = v + 'px'; setEditingThemeStyle({ ...editingThemeStyle, decorations: u }); } }} className="w-full bg-gray-950 border border-gray-700 rounded p-1 text-white" />
                          </div>
                          <div>
                            <label className="text-gray-400 mb-1 block">Right (Sağ)</label>
                            <input type="text" placeholder="örn: -10px" value={dec.right || ''} onChange={e => { const u = [...editingThemeStyle.decorations!]; u[idx].right = e.target.value; setEditingThemeStyle({ ...editingThemeStyle, decorations: u }); }} onBlur={e => { let v = e.target.value.trim(); if(v && !isNaN(Number(v))) { const u = [...editingThemeStyle.decorations!]; u[idx].right = v + 'px'; setEditingThemeStyle({ ...editingThemeStyle, decorations: u }); } }} className="w-full bg-gray-950 border border-gray-700 rounded p-1 text-white" />
                          </div>
                          <div>
                            <label className="text-gray-400 mb-1 block">Bottom (Alt)</label>
                            <input type="text" placeholder="örn: auto" value={dec.bottom || ''} onChange={e => { const u = [...editingThemeStyle.decorations!]; u[idx].bottom = e.target.value; setEditingThemeStyle({ ...editingThemeStyle, decorations: u }); }} onBlur={e => { let v = e.target.value.trim(); if(v && !isNaN(Number(v))) { const u = [...editingThemeStyle.decorations!]; u[idx].bottom = v + 'px'; setEditingThemeStyle({ ...editingThemeStyle, decorations: u }); } }} className="w-full bg-gray-950 border border-gray-700 rounded p-1 text-white" />
                          </div>
                          <div>
                            <label className="text-gray-400 mb-1 block">Left (Sol)</label>
                            <input type="text" placeholder="örn: 5px" value={dec.left || ''} onChange={e => { const u = [...editingThemeStyle.decorations!]; u[idx].left = e.target.value; setEditingThemeStyle({ ...editingThemeStyle, decorations: u }); }} onBlur={e => { let v = e.target.value.trim(); if(v && !isNaN(Number(v))) { const u = [...editingThemeStyle.decorations!]; u[idx].left = v + 'px'; setEditingThemeStyle({ ...editingThemeStyle, decorations: u }); } }} className="w-full bg-gray-950 border border-gray-700 rounded p-1 text-white" />
                          </div>
                          <div>
                            <label className="text-gray-400 mb-1 block">Genişlik (Width)</label>
                            <input type="text" placeholder="örn: 60px" value={dec.width || ''} onChange={e => { const u = [...editingThemeStyle.decorations!]; u[idx].width = e.target.value; setEditingThemeStyle({ ...editingThemeStyle, decorations: u }); }} onBlur={e => { let v = e.target.value.trim(); if(v && !isNaN(Number(v))) { const u = [...editingThemeStyle.decorations!]; u[idx].width = v + 'px'; setEditingThemeStyle({ ...editingThemeStyle, decorations: u }); } }} className="w-full bg-gray-950 border border-gray-700 rounded p-1 text-white" />
                          </div>
                          <div>
                    ...
                    {editingThemeStyle.profileDecorations?.map((dec, idx) => (
                      <div key={dec.id || idx} className="bg-gray-900 border border-gray-700 p-3 rounded-lg relative">
                        ...
                        <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
                          <div>
                            <label className="text-[10px] text-gray-400 font-bold">Top</label>
                            <input type="text" placeholder="örn: 5%" value={dec.top || ''} onChange={e => { const u = [...editingThemeStyle.profileDecorations!]; u[idx].top = e.target.value; setEditingThemeStyle({ ...editingThemeStyle, profileDecorations: u }); }} onBlur={e => { let v = e.target.value.trim(); if(v && !isNaN(Number(v))) { const u = [...editingThemeStyle.profileDecorations!]; u[idx].top = v + 'px'; setEditingThemeStyle({ ...editingThemeStyle, profileDecorations: u }); } }} className="w-full bg-gray-950 border border-gray-700 rounded p-1 text-white" />
                          </div>
                          <div>
                            <label className="text-[10px] text-gray-400 font-bold">Right</label>
                            <input type="text" placeholder="örn: 5%" value={dec.right || ''} onChange={e => { const u = [...editingThemeStyle.profileDecorations!]; u[idx].right = e.target.value; setEditingThemeStyle({ ...editingThemeStyle, profileDecorations: u }); }} onBlur={e => { let v = e.target.value.trim(); if(v && !isNaN(Number(v))) { const u = [...editingThemeStyle.profileDecorations!]; u[idx].right = v + 'px'; setEditingThemeStyle({ ...editingThemeStyle, profileDecorations: u }); } }} className="w-full bg-gray-950 border border-gray-700 rounded p-1 text-white" />
                          </div>
                          <div>
                            <label className="text-[10px] text-gray-400 font-bold">Bottom</label>
                            <input type="text" placeholder="örn: auto" value={dec.bottom || ''} onChange={e => { const u = [...editingThemeStyle.profileDecorations!]; u[idx].bottom = e.target.value; setEditingThemeStyle({ ...editingThemeStyle, profileDecorations: u }); }} onBlur={e => { let v = e.target.value.trim(); if(v && !isNaN(Number(v))) { const u = [...editingThemeStyle.profileDecorations!]; u[idx].bottom = v + 'px'; setEditingThemeStyle({ ...editingThemeStyle, profileDecorations: u }); } }} className="w-full bg-gray-950 border border-gray-700 rounded p-1 text-white" />
                          ...
                          <div>
                            <label className="text-[10px] text-gray-400 font-bold">Width</label>
                            <input type="text" placeholder="örn: 150px" value={dec.width || ''} onChange={e => { const u = [...editingThemeStyle.profileDecorations!]; u[idx].width = e.target.value; setEditingThemeStyle({ ...editingThemeStyle, profileDecorations: u }); }} onBlur={e => { let v = e.target.value.trim(); if(v && !isNaN(Number(v))) { const u = [...editingThemeStyle.profileDecorations!]; u[idx].width = v + 'px'; setEditingThemeStyle({ ...editingThemeStyle, profileDecorations: u }); } }} className="w-full bg-gray-950 border border-gray-700 rounded p-1 text-white" />
                          </div>
                          <div>
                            <label className="text-[10px] text-gray-400 font-bold">Rotation</label>
                ...
                <div className="space-y-1.5 pt-2">
                  ...
                  <div className={`relative rounded-2xl overflow-hidden p-4 border border-purple-500/30 bg-gray-950 shadow-xl space-y-3 min-h-[120px] flex flex-col justify-end ${editingThemeStyle.cardClass || ''}`}>
                    {editingThemeStyle.cardBgImageUrl && (
                      <div
                ...
                <div className="space-y-1.5 pt-6">
                  ...
                  <div className="relative min-h-[450px] sm:min-h-[550px] flex flex-col items-center justify-center pt-32 sm:pt-48 pb-20 sm:pb-32 px-4 overflow-hidden rounded-3xl isolate border border-white/5 bg-gray-950 shadow-2xl mt-3">
                    {/* Background Layer */}
                    <div 
      ...
      {activeTab === 'announcements-notifications' && (
        <div className="bg-gray-900/95 border border-purple-500/20 rounded-3xl p-4 sm:p-6 lg:p-8 shadow-xl">
