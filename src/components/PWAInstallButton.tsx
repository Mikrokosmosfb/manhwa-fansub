...
export const PWAInstallButton: React.FC<PWAInstallButtonProps> = ({ variant = 'header' }) => {
  ...
  const handleInstallClick = async () => {
    ...
    // 2. If in iframe or prompt not ready, open directly in top-level tab so browser can show native install dialog
    if (isInIframe) {
      window.open(window.location.href, '_blank');
      return;
    }

    // 3. Fallback: Show brief instant toast
    setShowToast(true);
    setTimeout(() => setShowToast(false), 4000);
  };
    ...
    <div className="fixed bottom-6 right-6 z-[999] flex items-center gap-3 px-5 py-3.5 rounded-2xl bg-gradient-to-r from-purple-900 via-purple-950 to-indigo-950 border border-purple-400/50 text-white shadow-2xl shadow-purple-950/80 animate-bounce-short">
      ...
      <div className="text-xs">
        ...
        <p className="text-purple-200/90 text-[11px] font-medium mt-0.5">
          Tarayıcınızın yükleme penceresinden onay vererek uygulamayı kullanabilirsiniz.
        </p>
      </div>
    </div>
  );
        ...
        >
          <div className="flex items-center gap-2.5">
            ...
            <div className="text-left">
              ...
              <div className="text-[10px] text-purple-300/70">Tek tıkla doğrudan cihazına yükle</div>
            </div>
          </div>
  ...
  return (
    <>
      <button
        onClick={handleInstallClick}
        title="Mikrokosmos Uygulamasını Doğrudan İndir"
