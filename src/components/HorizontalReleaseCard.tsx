...
export const HorizontalReleaseCard: React.FC<HorizontalReleaseCardProps> = ({ series, maxChapters = 1 }) => {
  ...
  return (
    <div className="bg-gray-900/90 border border-purple-500/20 hover:border-purple-500/40 rounded-3xl p-3.5 sm:p-4 flex gap-3.5 sm:gap-5 shadow-xl transition-all duration-300 hover:shadow-purple-900/20 group">
      
      {/* Left: Cover Poster with Badges */}
      <div 
        onClick={() => setView({ type: 'series-detail', seriesId: series.id })}
        className="relative w-28 sm:w-36 md:w-40 aspect-[2/3] flex-shrink-0 rounded-2xl overflow-hidden cursor-pointer shadow-lg bg-gray-950 border border-purple-500/20 self-start sm:self-center"
      >
        <img
        ...
        {/* Top Left Badges (Yeni / Sıcak / Güncel) */}
        <div className="absolute top-2 left-2 flex flex-col items-start gap-1 z-10">
          {series.isNew && (
            <span className="bg-gradient-to-r from-purple-600 via-fuchsia-500 to-indigo-600 text-white font-black text-[10px] px-2 py-0.5 rounded-full shadow-lg shadow-purple-900/80 border border-purple-300/40 animate-pulse flex items-center gap-0.5">
              <Sparkles size={10} className="text-purple-200 fill-purple-200" />
              YENİ
            </span>
          )}
          {!series.isNew && series.isHot && (
            <span className="bg-orange-600 text-white font-black text-[10px] px-2 py-0.5 rounded-md shadow-md">
              Sıcak
            </span>
          )}
          {(series.isGuncel || series.status === 'Güncel') && (
            <span className="bg-gradient-to-r from-cyan-600 via-sky-600 to-blue-600 text-white font-black text-[9px] px-1.5 py-0.5 rounded-md shadow-md border border-cyan-300/40 flex items-center gap-0.5">
        ...
        {(series.is18Plus || series.ageRating === '18+' || series.genres.includes('18+')) && (
          <span className="absolute top-2 right-2 bg-rose-600 text-white font-black text-[9px] px-1.5 py-0.5 rounded-md shadow-md border border-rose-300/40">
            18+
          </span>
        )}
