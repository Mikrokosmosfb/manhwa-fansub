...
export const NotificationsView: React.FC = () => {
  ...
  const totalFollowedNotifications = currentNotifications.filter(n => n.type === 'chapter' && isSeriesFollowed(n.seriesId)).length;
  const totalChapterNotifications = totalFollowedNotifications;
  const totalSystemNotifications = currentNotifications.filter(n => n.type !== 'chapter').length;
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative z-10">
          ...
          <div className="flex items-center gap-2 text-center text-xs font-bold">
            <div className="bg-gray-950/80 border border-purple-500/30 px-3.5 py-2 rounded-2xl">
              <span className="block text-lg font-extrabold text-white">{currentNotifications.length}</span>
            ...
            >
              ...
              {(notif.type === 'chapter' || notif.type === 'new-series' || notif.coverImage) && notif.coverImage ? (
                <div
                  ...
                  className="w-16 h-22 sm:w-20 sm:h-28 rounded-xl overflow-hidden flex-shrink-0 border border-purple-500/30 cursor-pointer shadow-md group-hover:scale-102 transition relative"
                >
                  <img
                  ...
                  />
                  {notif.type === 'new-series' && (
                    <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 to-transparent py-0.5 text-center">
                      <span className="text-[9px] font-black text-pink-300">YENİ SERİ</span>
                    </div>
                  )}
                </div>
              ) : (
              ...
              <div className="flex-1 min-w-0 pr-6 sm:pr-0">
                <div className="flex items-center gap-2 flex-wrap">
                  {notif.type === 'new-series' && (
                    <span className="px-2 py-0.5 rounded-md bg-gradient-to-r from-pink-900/90 to-purple-900/90 text-pink-200 text-xs font-black border border-pink-500/40">
                      🎉 YENİ SERİ
                    </span>
                  )}
