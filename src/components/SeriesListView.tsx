...
import { Library, Search, ArrowUpDown, LayoutGrid, List, Sparkles, BookOpen, Filter, X, RefreshCw } from 'lucide-react';
...
export const SeriesListView: React.FC = () => {
  ...
  // Extract all genres and custom badges across the entire site catalog & predefined list
  const genreCounts = useMemo(() => {
    const counts: Record<string, number> = {};

    // 1. Initialize all predefined genres
    ...
    // 2. Tally actual occurrences from series genres & custom badges
    ...
    // 3. Sort: Active counts first (descending), then alphabetically for others
  ...
  const sorted = useMemo(() => {
    ...
    if (sortBy === 'title') return [...filtered].sort((a, b) => a.title.localeCompare(b.title, 'tr'));
    if (sortBy === 'chapters') return [...filtered].sort((a, b) => b.chapters.length - a.chapters.length);
    if (sortBy === 'updated' || sortBy === 'newest') {
      return sortSeriesByLatestRelease(filtered);
    }
    return sortSeriesByLatestRelease(filtered);
  }, [filtered, sortBy]);
