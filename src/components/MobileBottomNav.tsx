...
import { Home, BookOpen, Bookmark, Calendar, Award, User } from 'lucide-react';
import { UserProfileModal } from './UserProfileModal';

export const MobileBottomNav: React.FC = () => {
  const { view, setView, bookmarks, user, openAuthModal } = useApp();
    ...
    <>
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-gray-950/95 border-t border-purple-500/30 backdrop-blur-lg px-2 py-1.5 shadow-2xl flex items-center justify-around text-[10px]">
        ...
        <button
          ...
          className={`flex flex-col items-center gap-0.5 py-1 px-2.5 rounded-xl transition ${
            isActive('schedule')
              ? 'text-amber-400 font-extrabold bg-amber-950/80 border border-amber-500/40'
              : 'text-gray-400 hover:text-white'
          }`}
        >
          <Calendar size={18} />
        ...
        {user ? (
          <button
            ...
            className="flex flex-col items-center gap-0.5 py-1 px-2.5 rounded-xl transition text-amber-300 hover:text-amber-200"
          >
