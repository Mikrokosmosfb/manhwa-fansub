...
export const NotificationsDropdown: React.FC<NotificationsDropdownProps> = ({ isOpen, onClose }) => {
  ...
  const followedNotificationsCount = currentNotifications.filter(n => n.type === 'chapter' && isSeriesFollowed(n.seriesId)).length;
  const chapterNotificationsCount = followedNotificationsCount; // Only show followed chapters
  const systemNotificationsCount = currentNotifications.filter(n => n.type !== 'chapter').length;

  // Filter notifications according to active tab
  const filteredNotifications = currentNotifications.filter(n => {
    if (n.type === 'chapter' && n.seriesId && !isSeriesFollowed(n.seriesId)) {
      return false;
    }
    if (activeTab === 'unread') {
  ...
  return (
    <div
      ref={dropdownRef}
      className="fixed inset-x-2 top-[72px] sm:absolute sm:inset-auto sm:right-0 sm:top-full sm:mt-2 w-auto sm:w-[420px] bg-gray-950/95 border border-purple-500/40 rounded-2xl shadow-2xl backdrop-blur-xl z-[100] overflow-hidden flex flex-col text-gray-100 animate-fadeIn"
    ...
    >
      ...
      {/* Filter Tabs */}
      <div className="flex items-center px-3 py-2 bg-gray-900/90 border-b border-purple-900/30 text-xs font-bold gap-1 overflow-x-auto scrollbar-none flex-shrink-0">
        <div className="flex items-center gap-1 min-w-max">
          ...
          >
            Tümü ({currentNotifications.length})
          </button>
          <button
        ...
        {filteredNotifications.length === 0 ? (
          ...
          filteredNotifications.map(notif => {
            ...
            >
              ...
              {(notif.type === 'chapter' || notif.type === 'new-series' || notif.coverImage) && notif.coverImage ? (
                <div className="relative w-11 h-15 rounded-lg overflow-hidden flex-shrink-0 border border-purple-500/30 shadow-md">
                  ...
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent flex items-end justify-center p-0.5">
                    <span className={`text-[8px] font-black ${notif.type === 'new-series' ? 'text-pink-300' : 'text-amber-300'}`}>
                      {notif.type === 'new-series' ? 'YENİ SERİ' : (notif.chapterNumber ? `BÖL. ${notif.chapterNumber}` : (notif.chapterTitle ? notif.chapterTitle.toUpperCase() : 'YENİ BÖLÜM'))}
                    </span>
                  </div>
                </div>
              ) : (
              ...
              <div className="flex-1 min-w-0 pr-4">
                <div className="flex items-center gap-1.5 flex-wrap">
                  ...
                  {notif.chapterNumber !== undefined ? (
                    ...
                    <span className="px-1.5 py-0.2 rounded bg-purple-800 text-purple-200 text-[9px] font-mono font-black truncate max-w-[80px]">
                      {notif.chapterTitle}
                    </span>
                  ) : null}
